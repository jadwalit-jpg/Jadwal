...
describe('AdminService.markPayoutsPaid — in-flight guard', () => {
  test('blocks when a vendor has a PENDING request covering the submitted payments', async () => {
    ...
    ctx.prisma._client.payment.findMany.mockResolvedValueOnce([
      { id: 'p1', booking: { vendor: { id: 'vA', businessNameEn: 'Alpha Tours' } } },
    ]);
    ...
    await expect(ctx.sut.markPayoutsPaid(['p1']))
  ...
  test('blocks when the specific paymentId is in an APPROVED request.paymentIds[]', async () => {
    const ctx = await buildSut();
    ctx.prisma._client.payment.findMany.mockResolvedValueOnce([
      { id: 'p1', booking: { vendor: { id: 'vA', businessNameEn: 'Alpha' } } },
      { id: 'p2', booking: { vendor: { id: 'vA', businessNameEn: 'Alpha' } } },
    ]);
    ...
    await expect(ctx.sut.markPayoutsPaid(['p1', 'p2']))
  ...
  test('allows bulk mark-paid when APPROVED request covers different paymentIds', async () => {
    ...
    const result = await ctx.sut.markPayoutsPaid(['p1']);
  ...
  test('names up to 3 offending vendors and appends "and N more" for the rest', async () => {
    ...
    ctx.prisma._client.payment.findMany.mockResolvedValueOnce([
      { id: 'p1', booking: { vendor: { id: 'v1', businessNameEn: 'V One' } } },
      { id: 'p2', booking: { vendor: { id: 'v2', businessNameEn: 'V Two' } } },
      { id: 'p3', booking: { vendor: { id: 'v3', businessNameEn: 'V Three' } } },
      { id: 'p4', booking: { vendor: { id: 'v4', businessNameEn: 'V Four' } } },
      { id: 'p5', booking: { vendor: { id: 'v5', businessNameEn: 'V Five' } } },
    ]);
    ctx.prisma._client.payoutRequest.findMany.mockResolvedValueOnce([
    ...
    await expect(ctx.sut.markPayoutsPaid(['p1', 'p2', 'p3', 'p4', 'p5']))
  ...
  test('notifies each unique vendor once with a slug-scoped link', async () => {
    ...
    await ctx.sut.markPayoutsPaid(['p1', 'p2']);
...
describe('AdminService.processPayoutRequest — APPROVED → COMPLETED (two-step money-settle)', () => {
  test('closing the request does NOT call payment.updateMany (payments stay UNPAID)', async () => {
    ...
    // Inside the $transaction callback — the shared makePrismaMock passes the
    // same client through, so we prime the mocks used inside:
    //  - payment.count returns `paymentIds.length` → still eligible.
    //  - payoutRequest.updateMany flips the request to COMPLETED.
    ctx.prisma._client.payment.count.mockResolvedValueOnce(2);
  ...
  test('aborts if any locked payment drifted out of SUCCESS+UNPAID between approve and complete', async () => {
    const ctx = await buildSut();
    ctx.prisma._client.payoutRequest.findUnique.mockResolvedValueOnce({
    ...
    // Only 1 of 2 still eligible → admin markPayoutsPaid may have run in the
    // gap, or a cascade refund moved one out. Abort for admin to investigate.
    ctx.prisma._client.payment.count.mockResolvedValueOnce(1);

    await expect(ctx.sut.processPayoutRequest('req-1', 'COMPLETED'))
      .rejects.toThrow(/no longer eligible.*review/i);
...
describe('AdminService.getPayouts — inflightRequest enrichment', () => {
  ...
  test('PENDING request on a vendor marks ALL that vendor\'s rows with {status:"PENDING"}', async () => {
    const ctx = await buildSut();
    seedCommon(ctx, [
      { id: 'p1', status: 'SUCCESS', booking: { vendor: { id: 'vA', businessNameEn: 'Alpha' } } },
      { id: 'p2', status: 'SUCCESS', booking: { vendor: { id: 'vA', businessNameEn: 'Alpha' } } },
      { id: 'p3', status: 'SUCCESS', booking: { vendor: { id: 'vB', businessNameEn: 'Beta' } } },
    ]);
    ctx.prisma._client.payoutRequest.findMany.mockResolvedValueOnce([
