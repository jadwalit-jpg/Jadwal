...
describe('pay2m allowlist helper — PROD §18.7', () => {
  it('defaults to the production PAY2M origin', () => {
    ...
    // pay.pay2m.com, this allowlist must include that origin or
    // every real payment will be refused by the client.
    expect(PAY2M_ALLOWED_ORIGINS).toContain('https://pay.pay2m.com');
  });

  it('does NOT include the deprecated payments.pay2m.com origin', () => {
    // payments.pay2m.com was the legacy host. PAY2M migrated to
    // pay.pay2m.com; the old origin is intentionally not allowlisted
    // so a tampered API response that points there gets rejected.
    expect(PAY2M_ALLOWED_ORIGINS).not.toContain('https://payments.pay2m.com');
  });
  ...
  describe('accepts legitimate PAY2M URLs', () => {
    ...
    it('accepts other paths on the same origin (PAY2M can change paths)', () => {
      expect(isAllowedPay2mFormAction('https://pay.pay2m.com/any/other/path')).toBe(true);
    });

    it('accepts URLs with query strings and fragments', () => {
      expect(
        isAllowedPay2mFormAction('https://pay.pay2m.com/x?q=1#frag'),
      ).toBe(true);
    });
  });
  ...
  describe('rejects non-HTTPS schemes (no downgrade, no XSS surface)', () => {
    it('rejects http://', () => {
      expect(isAllowedPay2mFormAction('http://pay.pay2m.com/x')).toBe(false);
    });
  ...
  describe('rejects foreign origins', () => {
    it('rejects a look-alike subdomain of a known host', () => {
      expect(isAllowedPay2mFormAction('https://pay.pay2m.com.evil.com/x')).toBe(false);
    });
    ...
    it('rejects the deprecated payments.pay2m.com origin', () => {
      // Belt-and-suspenders: even if some intermediate forgets to update
      // and emits the old URL, the client should refuse to POST to it.
      expect(isAllowedPay2mFormAction('https://payments.pay2m.com/PostTransaction')).toBe(false);
    });
  ...
  describe('rejects tampered / malformed URLs', () => {
    it('rejects URLs with embedded credentials', () => {
      ...
      expect(
        isAllowedPay2mFormAction('https://user:pass@pay.pay2m.com/x'),
      ).toBe(false);
      expect(
        isAllowedPay2mFormAction('https://:password@pay.pay2m.com/x'),
      ).toBe(false);
      expect(
        isAllowedPay2mFormAction('https://user@pay.pay2m.com/x'),
      ).toBe(false);
    });
    ...
    it('rejects a protocol-relative URL (`//host/...`)', () => {
      expect(isAllowedPay2mFormAction('//pay.pay2m.com/x')).toBe(false);
    });
  });
});
