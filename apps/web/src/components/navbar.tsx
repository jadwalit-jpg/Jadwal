...
/**
 * The canonical navbar — used by every route in the app. Lighter than the
 * legacy navbar that lives at `_archive/navbar-legacy.tsx` (kept for
 * reference, never imported): no glass pill wrapping the desktop nav links,
 * no `backdrop-blur` on the header or the mobile menu, no bar-mounted
 * notification-bell popover. The notifications surface lives inside the menu
 * (a row that links to `/notifications`) on both mobile *and* the desktop
 * avatar dropdown — same shared `['notifications', 'unread-count']` cache
 * as everywhere else.
 *
 * `variant` prop:
 *   - `'transparent'` (default) — overlay style: transparent over the hero,
 *     opaque past it (`scrollY > innerHeight - 80`). Use on routes that
 *     start with a full-viewport hero (`/home`).
 *   - `'solid'` — always-opaque. Skip the scroll listener. Use everywhere
 *     else (`/explore`, `/bookings`, `/likes`, `/profile`, `/notifications`,
 *     activity / contact / auth flows, etc.).
 *
 * Mobile menu items each have a leading lucide icon (Home / Compass / Tag /
 * Store / Bell / Globe / etc.). The mobile menu is a floating overlay with
...
export default function Navbar({ variant = 'transparent' }: { variant?: 'transparent' | 'solid' } = {}) {
  ...
  useEffect(() => {
    // `variant='solid'` is always-opaque — skip the listener entirely so we
    // don't burn cycles on a scroll event we won't act on. Only `transparent`
    // (hero overlay) needs to flip the header to opaque past the viewport.
  ...
  // The single source of truth for "navbar is opaque". `variant='solid'`
  // forces it on; `variant='transparent'` follows the scroll state. All
  // visual class branches below use this — never `scrolled` directly.
  ...
  // Tone the inline link / icon-button colors to the opaque state. On
  // `variant='solid'` we never go transparent, so these always pick the
  // gray-on-white treatment. On `variant='transparent'` they flip with the
  // scroll position via `isOpaque`.
            ...
            <div ref={userMenuRef} className="relative hidden md:block">
              ...
              >
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                  isOpaque ? 'bg-sky-600 text-white dark:bg-blue-600' : 'bg-white/20 text-white'
                }`}>
                  {user.fullName?.charAt(0).toUpperCase() ?? 'U'}
                </span>
                <span className={`text-sm font-medium max-w-[100px] truncate ${
                  isOpaque ? 'text-gray-800 dark:text-white' : 'text-white'
                }`}>
                ...
                <ChevronDown
                  ...
                  className={`w-3.5 h-3.5 transition-transform ${userMenuOpen ? 'rotate-180' : ''} ${
                    isOpaque ? 'text-gray-400 dark:text-slate-500' : 'text-white/60'
                  }`}
                />
              </button>
              {userMenuOpen && (
                ...
                >
                  ...
                  <div className="py-1.5">
                    {/* Notifications row — top of the dropdown so desktop
                        customers have a parallel surface to the mobile menu
                        row. Same `/notifications` destination and same
                        shared `useQuery` cache for the unread badge. */}
                    <Link
                      href="/notifications"
                      onClick={() => setUserMenuOpen(false)}
                      className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 hover:text-sky-600 dark:hover:text-white transition-colors"
                    >
                      <span className="flex items-center gap-3">
                        <Bell aria-hidden="true" className="w-4 h-4 text-gray-400 dark:text-slate-500" />
