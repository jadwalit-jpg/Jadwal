...
export default function EditActivityPage() {
  ...
  // Fetch activity data by slug
  const { data: activity, isLoading: activityLoading } = useQuery({
    queryKey: [user?.id, 'vendor-activity', activitySlug],
    queryFn: () => api.get(`/vendor/activities/by-slug/${activitySlug}`).then(r => r.data),
  ...
  // Filter cities by vendor's country
  ...
  // Pre-populate form from fetched activity
  useEffect(() => {
    if (activity && !initialized) {
      setForm({
        ...
        bookingType: activity.bookingType ?? 'HOURLY',
      ...
      setExtraServices(Array.isArray(activity.extraServices)
        ? activity.extraServices.map((s: any) => ({ name: s.name, price: s.price ?? 0, perPerson: !!s.perPerson }))
        : []);
      ...
      if (activity.hasUnits) {
        setHasUnits(true);
        setUnitCount(activity.unitCount != null ? String(activity.unitCount) : '');
        setUnitCapacity(activity.unitCapacity != null ? String(activity.unitCapacity) : '');
      }
  ...
  const updateMutation = useMutation({
    ...
    onError: (err) => {
      toast(getApiError(err, t('vendor.activities.wizard.toast.updateFailed')), 'error');
    },
  });
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 3) {
      ...
      if (!hasUnits && form.capacity && Number(form.capacity) > 10000) {
  ...
  const handleTitleEnChange = (val: string) => {
    ...
    // Do NOT auto-update slug on edit — slug is readonly and already set
  };
  ...
  // Loading skeleton
  if (activityLoading || !initialized) {
    return (
      <div className="min-h-screen bg-stone-50 dark:bg-slate-950 font-outfit">
        ...
        <main className="ms-64 min-h-screen">
          ...
          <div className="px-10 py-8 space-y-6 max-w-4xl">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-48 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl animate-pulse" />
            ))}
          </div>
        </main>
      </div>
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-slate-950 font-outfit">
      ...
      <main className="ms-64 min-h-screen">
        ...
        <div className="flex items-center justify-between px-10 py-6 border-b border-stone-200 dark:border-slate-800 bg-stone-50 dark:bg-slate-950">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('vendor.activities.wizard.ui.editTitle')}</h1>
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">{t('vendor.activities.wizard.ui.editSubtitle')}</p>
          </div>
          <button
            type="button"
            onClick={() => router.push(`/vendor/${vendorSlug}/activities`)}
          ...
          >
            <X className="h-4 w-4" />
            {t('vendor.activities.wizard.ui.cancel')}
          </button>
        </div>

        {/* Active activity warning banner */}
        {activity?.status === 'ACTIVE' && (
          <div className="mx-10 mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl text-sm text-amber-700 dark:text-amber-400 flex items-start gap-3">
            <AlertTriangle className="h-5 w-5 mt-0.5 shrink-0" />
            <div>
              <p className="font-semibold">{t('vendor.activities.wizard.ui.activeBannerTitle')}</p>
              <p className="mt-0.5">{t('vendor.activities.wizard.ui.activeBannerDetail')}</p>
            </div>
          </div>
        )}
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 dark:border-slate-800">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                    {t('vendor.activities.wizard.ui.unitsTitle')}{' '}
                    <span className="text-sm font-normal text-gray-400 dark:text-slate-500">{t('vendor.activities.wizard.ui.unitsOptionalMark')}</span>
                  </h3>
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-3 gap-8">
                <div className="col-span-2 space-y-5">
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    {!hasUnits && (
                      <FieldGroup label={t('vendor.activities.wizard.ui.fieldMaxGuestsPerBooking')} error={errors.capacity} hint={t('vendor.activities.wizard.ui.hintMaxGuestsUnlimited')}>
                        <input type="number" min="1" value={form.capacity}
                          onChange={e => updateField('capacity', e.target.value)}
                          className={inputCls(!!errors.capacity)} placeholder={t('vendor.activities.wizard.ui.phMaxGuestsExample')} />
                      </FieldGroup>
                    )}
                  </div>
        ...
        <div className="px-10 py-5 border-t border-stone-200 dark:border-slate-800 bg-stone-50 dark:bg-slate-950 flex items-center justify-between w-full">
          ...
          {currentStep < STEPS.length ? (
            ...
            >
              ...
              {t('vendor.activities.wizard.ui.saveChanges')}
            </button>
          )}
        </div>
      </main>
