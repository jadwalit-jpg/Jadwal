...
/**
 * `/home` — the marketplace homepage. This is the perf-readied build (built
 * iteratively as `/home-test` and swapped in here); the legacy heavier build
 * is parked at `app/_archive/home-page.tsx` and is NOT routed (`_archive` is
 * a private Next.js App Router folder), kept only for reference.
 *
 * Compared to the legacy `/home` build:
 *  - `<NavbarBasic/>` (overlay header, scroll-to-opaque past the hero) instead
 *    of the full `<Navbar/>` — no admin/vendor pills on the bar, no glass-pill
 *    nav-link wrappers, no `md:backdrop-blur-xl` on the header. Mobile menu is
